...
namespace friday.core.repositories
{
    public class HouseRepository : Repository<House>, friday.core.repositories.IHouseRepository
    {
        ...
        {
            var s = (from x in this.Session.Query<House>() select x).Where(o => o.Rent.Id == rentID && o.GlobalGoodsType.Id == merchantGoodTypeID && o.IsDelete == false).OrderByDescending(o => o.MonthAmount).Skip(start).Take(limit).ToList();
            total = (from x in this.Session.Query<House>() select x).Where(o => o.Rent.Id == rentID && o.GlobalGoodsType.Id == merchantGoodTypeID && o.IsDelete == false).OrderByDescending(o => o.MonthAmount).Count();
            return s;
        }
        public IList<House> GetHouseByRentIDAndKeywordAndBetweenPriceOrderBy(string rentID, string keyword, double price1, double price2,string goodTypeId, string orderType, int start, int limit, out int total)
        {
            ...
            var s = (from x in this.Session.Query<House>() select x).Where(o => o.Rent.Id == rentID && o.Name.Contains(keyword) && (o.Price >= price1 || price1 == -1) && (o.Price <= price2 || price2 == -1) && (o.GlobalGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).OrderByDescending(o => o.MonthAmount).Skip(start).Take(limit).ToList();
            total = (from x in this.Session.Query<House>() select x).Where(o => o.Rent.Id == rentID && o.Name.Contains(keyword) && (o.Price >= price1 || price1 == -1) && (o.Price <= price2 || price2 == -1) && (o.GlobalGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).Count();
                return s;
