...
namespace friday.core.repositories
{
    public class FoodRepository : Repository<Food>, friday.core.repositories.IFoodRepository
    {
        ...
        public IList<Food> GetFoodByRestaurantIDAndKeywordAndBetweenPriceOrderBy(string restaurantID, string keyword, double price1, double price2,string goodTypeId, string orderType, int start, int limit, out int total)
        {
            ...
            {
                var s = (from x in this.Session.Query<Food>() select x).Where(o => o.Restaurant.Id == restaurantID && o.Name.Contains(keyword) && o.Price >= price1 && o.Price <= price2 && (o.MerchantGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).OrderByDescending(o => o.MonthAmount).Skip(start).Take(limit).ToList();
                total = (from x in this.Session.Query<Food>() select x).Where(o => o.Restaurant.Id == restaurantID && o.Name.Contains(keyword) && o.Price >= price1 && o.Price <= price2 && (o.MerchantGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).Count();
                return s;
            }
            else if (price1 == -1 && price2 == -1)
            {
                var s = (from x in this.Session.Query<Food>() select x).Where(o => o.Restaurant.Id == restaurantID && o.Name.Contains(keyword) && (o.MerchantGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).OrderByDescending(o => o.MonthAmount).Skip(start).Take(limit).ToList();
                total = (from x in this.Session.Query<Food>() select x).Where(o => o.Restaurant.Id == restaurantID && o.Name.Contains(keyword) && (o.MerchantGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).Count();
                return s;
            }
            else if (price1 == -1 && price2 != -1)
            {
                var s = (from x in this.Session.Query<Food>() select x).Where(o => o.Restaurant.Id == restaurantID && o.Name.Contains(keyword) && o.Price <= price2 && (o.MerchantGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).OrderByDescending(o => o.MonthAmount).Skip(start).Take(limit).ToList();
                total = (from x in this.Session.Query<Food>() select x).Where(o => o.Restaurant.Id == restaurantID && o.Name.Contains(keyword) && o.Price <= price2 && (o.MerchantGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).Count();
                return s;
            }
            else
            {
                var s = (from x in this.Session.Query<Food>() select x).Where(o => o.Restaurant.Id == restaurantID && o.Name.Contains(keyword) && o.Price >= price1 && (o.MerchantGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).OrderByDescending(o => o.MonthAmount).Skip(start).Take(limit).ToList();
                total = (from x in this.Session.Query<Food>() select x).Where(o => o.Restaurant.Id == restaurantID && o.Name.Contains(keyword) && o.Price >= price1 && (o.MerchantGoodsType.Id == goodTypeId || goodTypeId == "0") && o.IsDelete == false).Count();
                return s;
            }
