...
}(); /*pub-1|2013-03-15 11:27:58*/
(function(B) {
	var F = document,
		E = window,
		A = E.g_config,
		C = A.assetsHost || "http://l.tbcdn.cn";
	A.t = A.t + "d4";
	B.config({
		combine: true,
		map: [
	...
	B.config({
		modules: {
			...
			"malldetail/sku/buylink": {
				requires: ["template", "malldetail/sku/validator"]
			},
			"malldetail/sku/thumbViewer": {
				requires: ["dom", "event", "malldetail/common/util", "imagezoom"]
			},
			"malldetail/sku/skuFeatureIcon": {
				requires: ["dom"]
			},
			"malldetail/sku/sellCount": {
				requires: ["dom"]
			},
			"malldetail/sku/skuLade": {
				requires: ["dom", "event"]
			},
			...
			"malldetail/sku/skuAmount": {
				requires: ["malldetail/sku/validator"]
			},
			...
			"malldetail/sku/stock": {
				requires: ["template", "malldetail/sku/skuMsg"]
			},
			"malldetail/sku/linkbasket": {
				requires: ["anim", "malldetail/sku/skuMsg", "malldetail/sku/validator"]
			},
			"malldetail/sku/popupsimulate": {
				requires: ["malldetail/sku/validator"]
			},
			...
			"malldetail/sku/3c": {
				requires: ["malldetail/sku/areaSeletor", "malldetail/sku/freight", "malldetail/sku/stock", "malldetail/sku/skuMsg"]
			},
			"malldetail/sku/price": {
				requires: ["template"]
			},
			"malldetail/sku/service": {
				requires: ["template"]
			},
			"malldetail/sku/stat": {
				requires: ["dom", "event", "ajax"]
			...
			"malldetail/sku/calculator": {
				requires: ["dom", "event"]
			},
			"malldetail/combos/combos": {
				requires: ["cookie", "switchable", "malldetail/sku/areaSeletor", "json"]
			},
			"malldetail/shop/shop": {
				requires: ["malldetail/common/util"]
			},
			"malldetail/other/attributes": {
				requires: ["template"]
			},
			"malldetail/tabbar/tabbarAttr": {
				requires: ["template", "malldetail/tabbar/localData"]
			},
			"malldetail/other/staticMods": {
				requires: ["template", "malldetail/data/ajax", "json"]
			},
			"malldetail/common/tbskip": {
				requires: ["cookie", "swf", "json"]
			},
			"malldetail/sku/validator": {
				requires: ["malldetail/sku/skuMsg"]
			},
			"malldetail/sku/skuMsg": {
				requires: ["template"]
			},
			"malldetail/recommend/basketrecommend": {
				requires: ["malldetail/recommend/basketrecommend.css"]
			},
			"malldetail/sku/areaSeletor": {
				requires: ["overlay", "template"]
			},
			"malldetail/sku/regionSelectPopup": {
				requires: ["overlay"]
			},
			"malldetail/data/ajax": {
				requires: ["dom"]
			},
			"malldetail/other/relate": {
				requires: ["dom", "ajax"]
			},
			TMiniCart: {
	...
	B.mix(B, {
		...
		_sendImage: function(H, J) {
			...
			var I = {
				catid: B.cfg("itemDO").categoryId,
				itemId: B.cfg("itemDO").itemId,
				pagetype: this.getPageType(),
				rn: this.getUrlParams("rn"),
				sellerId: B.cfg("itemDO").userId
			};
		...
		onLogin: function(L, I) {
			...
			var H = B.mix({
				proxyURL: "http://detail." + K + "/cross/x_cross_iframe.htm?type=minilogin&t=" + B.t()
			}, I);
		...
		loadMdskip: function(L) {
			...
			var H = B.param(I),
				J = B.now();
			window.onMdskip = K;
			B.getScript(L + "&callback=onMdskip&" + H, {
				error: K
			})
		},
		Setup: function(H) {
		...
		Asyn: function() {
			...
			var H = {
				timeout: 30000,
				runCount: 0,
				jsonpCallback: "jsonp" + B.now() + "_" + B.guid(),
				pageCache: false
			};
			var L = {};
	...
	TShop.initFoot = function(H) {
		B.use(["dom", "malldetail/common/util", "malldetail/other/lazy", "malldetail/dc/dc", "malldetail/other/leftSlide", "malldetail/other/mainBody", "malldetail/other/init"], function(L, N, J) {
			...
			TShop.use("malldetail/dc/dc", function(U, T) {
				...
				if (R && R.itemDO) {
					Q.isvParams = U.mix({
						nickName: R.itemDO.sellerNickName,
						userId: R.itemDO.userId,
						shopId: "",
						itemId: "",
						itemNumId: R.itemDO.itemId,
						shopStats: R.itemDO.feature,
						validatorUrl: R.itemDO.validatorUrl,
						templateName: R.itemDO.templateName,
						templateId: R.itemDO.templateId
					}, R.isv)
				}
				T.init({
					wangpuConfig: Q
			...
			if (H.showShoplist) {
				L.use(["event", "dom"], function(T, W, X) {
					...
					var Q = location.href,
						U = "https://login.taobao.com/member/login.jhtml?TPL_redirect_url=" + Q,
						R = document.getElementById("J_Guestlogin");
					if (R) {
						R.href = U
					}
				})
