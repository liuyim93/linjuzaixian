    ...
    <div class="pageContent" layoutH="20">
        ...
        <div class="pageFormContent" >
            ...
            <script type="text/javascript">
                ...
                debugger
                var porImg = $('#Add_Activity_ImagePreview');
                    ...
                    //如果是IE浏览器，采用滤镜效果，进行显示，但特别注意的是该滤镜效果使用的对象是div对象，并非img对象，因此我们需要将原有的img对象remove同时生成新的div对象，并且赋值相应的class和id  
                        ...
                        $(ieImageDom).css({
                            ...
                            width: '360px',
                            height: '95px'
                        }).attr({ "id": "Add_Activity_ImagePreview" });
                        //删除原有img对象，append创建div的dom对象  
                        $(ieImageDom).insertAfter("#Add_Activity_Image");
                        porImg.remove();
                        //采用滤镜效果生成图片预览  
                        file.select();
            ...
            <script type="text/javascript">
                function activityPreviewSubImage(file) {
                    debugger
                    var porImg = $('#Add_Activity_SubImagePreview');
                    //判断该浏览器是否为w3c标准，既非IE浏览器   
                    if (file["files"] && file["files"][0]) {
                        //使用JavaScript的FileReader对象来读取本地数据，并且将数据结果赋值给image的src，具体该对象如何实现的还未深入研究  
                        var reader = new FileReader();
                    ...
                    //如果是IE浏览器，采用滤镜效果，进行显示，但特别注意的是该滤镜效果使用的对象是div对象，并非img对象，因此我们需要将原有的img对象remove同时生成新的div对象，并且赋值相应的class和id  
                        ...
                        $(ieImageDom).css({
                            ...
                            width: '360px',
                            height: '95px'
                        }).attr({ "id": "Add_Activity_SubImagePreview" });
                        //删除原有img对象，append创建div的dom对象  
                        $(ieImageDom).insertAfter("#Add_Activity_SubImage");
                        porImg.remove();
                        //采用滤镜效果生成图片预览  
                        file.select();
                        path = document.selection.createRange().text;
            ...
            <p style="clear:left;height: 150px">
                <label>
                    幻灯图片上传：</label>
                <input id="Add_Activity_Image" type="file" class="required textInput gray" runat="server" onchange="activityPreviewImage(this);"/>

                <img id="Add_Activity_ImagePreview" runat="server" style="margin:10px;width: 360px; height: 95px" />
            ...
            <p style="height: 150px">
                <label>
                    标题图片上传：</label>
                <input id="Add_Activity_SubImage" type="file" class="required textInput gray" runat="server"  onchange="activityPreviewSubImage(this);"/>

                <img  id="Add_Activity_SubImagePreview" runat="server" style="margin:10px;width: 360px; height: 95px" />
