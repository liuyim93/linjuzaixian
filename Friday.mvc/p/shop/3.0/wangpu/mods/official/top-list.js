KISSY.add("wangpu/mods/official/top-list", function (S, Core, Switchable, Overlay, Node) {
    ...
    S.augment(TopList, { _init: function () {
        ...
        var dataEl, self = this, triggers, panels, val, activeIndex = 0, tabs;
        if (DOM.get("ul.tab-nav", self._mod) == null) {
            return
        }
        var noPicUrl = "http://img.taobao.com/newshop/nopicture.gif", markup = '<li><div class="pic"><a target="_blank" href="#href#"><img width="40px" bigpicurl="#bigpic#" src="#src#"></a></div><div class="desc"><a target="_blank" title="#title#" href="#href#">#title#</a></div><div class="price"><i></i>#price#</div><div class="collecter"><i></i>\u6536\u85cf\u4eba\u6c14&nbsp;&nbsp;#colcount#</div></li>', pic_suffix = "_40x40.jpg", big_pic_suffix = "_120x120.jpg", rankpanel2 = DOM.query(".J_TopColl");
        var conf = DOM.get("#J_dcTopListData"), val = DOM.attr(conf, "data-value");
        if (conf && val) {
            val = eval("(" + val + ")")
        }
        if (!val) {
        ...
        var tabs_cfg = { triggers: triggers, panels: panels, delay: 0.5, activeTriggerCls: "selected" };
        if (triggers.length === 0) {
            tabs_cfg["hasTriggers"] = false
        }
        tabs = new Switchable.Tabs(DOM.get("div.rank-tab", self._mod), tabs_cfg);
        tabs.on("switch", function (ev) {
            var tg = ev.target, index = ev.currentIndex, curr = DOM.get(".J_TopColl", tg.content), rindex = -1;
            if (index == 1 && DOM.attr(curr, "init-data") != "true") {
                ...
                window.shop.rankCallback = function (data) {
                    var result = "", spm = DOM.attr(DOM.parent("#J_ranktab", 3), "data-spm");
        ...
        var self = this, popup;
        popup = new Overlay.Popup({ triggerType: "mouse" });
        var rankPanels = DOM.get(".rank-panels", self._mod), $ = Node.all;
        Event.delegate(rankPanels, "mouseenter", "div.pic", function (e) {
            var t = $(e.currentTarget);
            popup.set("content", '<img src="' + t.one("img").attr("bigpicurl") + '"/>');
            popup.set("align", { node: t, points: ["tr", "tl"], offset: [0, 0] });
            popup.show()
        });
